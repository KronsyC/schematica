import genId from "../lib/helpers/genCodeSafeId";

//TYPEADDITION
export type SchemaType =
    | "string"
    | "object"
    | "number"
    | "boolean"
    | "array"
    | "any";
export interface BaseSchemaTemplate{
    type: SchemaType;
    ref?: string;
    name?: string;
    nullable?:boolean;
}
/**
 * The Class from which all schemas are based from
 * T is the template Type
 */

export abstract class BaseSchema<T extends BaseSchemaTemplate> {
    type: SchemaType;

    /**
     * *[INTERNAL]* The id of the schema, this is autogenerated and used in codegen to avoid variable collisions
     */
    id: string;

   /**
    * The name of the schema, used in errors and such
    */
   name: string
    /**
    * The ref of the schema, used to reference it in other schemas
    */
    ref?:string;
    /**
     * The Template from which the schema is derived
     */
    template: T;
    /**
     * A kvp for different utils to persist data
     */
    cache: Map<string, any> = new Map()
    /**
     * Allow the Data to be an undefined value, defaults to false
     */
    nullable: boolean;
    abstract typecheck:string
    constructor(template:T){
        this.type = template.type
        this.template = template
        this.ref=template.ref
        this.id=genId(6)
        this.name = template.name || "Data"
        this.nullable = template.nullable ?? false
    }

    validateSchema(){}
}
